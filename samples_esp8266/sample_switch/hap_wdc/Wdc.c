...
static void read_write_flash(void)
{
    ...
    ret = spi_flash_read((PRIV_PARAM_START_SEC + PRIV_PARAM_SAVE) * SPI_FLASH_SEC_SIZE, values, 32);
    printf("read values: %s\n", (ret == SPI_FLASH_RESULT_OK) ? "OK" : "FAILED");
    ...
    ret = spi_flash_write((PRIV_PARAM_START_SEC + PRIV_PARAM_SAVE) * SPI_FLASH_SEC_SIZE, data, 32);
    printf("write data: %s\n", (ret == SPI_FLASH_RESULT_OK) ? "OK" : "FAILED");
    printf("data[8]: \n");
    for (i = 0; i < 8; ++i)
    {
